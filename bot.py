...
UPGRADE: Smart opening pakai AI + fallback DB diperbesar + anti-pengulangan
"""
...
# Anti-repeat: jangan ulangi balasan yang sama dalam N pesan terakhir
RECENT_HISTORY_SIZE = 25
...
# Anti-repeat memory (dipakai untuk reply cycle & self-chat)
recent_replies = deque(maxlen=RECENT_HISTORY_SIZE)
recent_openers = deque(maxlen=15)
...
# Diperbesar jauh supaya variasi tinggi kalau Gemini down.
# Dipakai untuk MEMBALAS chat orang lain.

FALLBACK_RESPONSES = [
    # AGREEMENT
    "Haha iya", "Wkwk bener", "Bet bro", "Iyah deh", "Setuju banget",
    "Sama sih", "True true", "Hehe iya", "Okayy bro", "Bener banget",
    "Nah iya tuh", "Wkwk sama", "Iya juga sih", "Bener tuh", "Cocok bro",
    "Setuju sih", "Iyaa bener", "Sama gue juga", "Emang bener", "Nah gitu",
    "Iya sih emang", "Wkwk cocok", "Bener adanya", "Yoi bro", "Mantap setuju",

    # HUMOR
    "Njir kocak", "Wkwk gila", "Gila banget", "Kocak parah", "Gokil bro",
    "Lucu banget", "Ngakak bro", "Wkwk asli", "Haha ngakak", "Parah ini",
    "Astaga kocak", "Ngakak sih", "Gila kocak", "Wkwk anjay", "Kacau bro",
    "Ngakak parah", "Kocak sumpah", "Wkwk mantap", "Gila lucu", "Absurd bro",
    "Ngakak abis", "Kocak gila", "Wkwk parah", "Anjir kocak", "Ampun kocak",

    # SURPRISE
    "Waduh bro", "Asli gak?", "Beneran nih?", "Serius bro?", "Kok bisa?",
    "Gimana ini?", "Wah wah", "Astaga", "Sumpah bro?", "Apa nih?",
    "Loh kok gitu?", "Hah serius?", "Beneran tuh?", "Waduh gimana", "Kaget gue",
    "Anjir beneran?", "Wah gila", "Kok gitu sih", "Astaga bro", "Hah kok bisa",
    "Yakin nih?", "Waduh parah", "Kaget banget", "Hah ternyata", "Wah serius",

    # QUESTION
    "Apa sih?", "Gimana?", "Siapa itu?", "Cerita dong", "Lanjut", "Terus apa?",
    "Trus?", "Gimana nih?", "Apa dong?", "Info dong",
    "Kok bisa gitu?", "Maksudnya apa?", "Terus gimana?", "Kenapa emang?", "Dimana itu?",
    "Kapan tuh?", "Ada apa emang?", "Kok bisa sih?", "Trus kenapa?", "Emang kenapa?",
    "Ceritain dong", "Lanjutin dong", "Terus terus?", "Habis itu apa?", "Nah terus?",

    # SUPPORT
    "Kuat bro", "Semangat", "Bisa kok", "Yakin bisa", "Go go", "Push terus",
    "Ayuuu", "Semangat teman", "Pasti bisa", "Sukses bro",
    "Semangat terus", "Kuat ya bro", "Yakin aja", "Gaskeun bro", "Pasti sukses",
    "Tetap semangat", "Jangan nyerah", "Kamu bisa", "Fighting bro", "Sabar ya",
    "Kuat kuat aja", "Semangat pagi", "Ayo bisa kok", "Terus maju", "Optimis aja",

    # OPENING (self-chat / mancing obrolan)
    "Woi sepi banget", "Gimana kabar bro?", "Ada yang seru?", "Gas ngobrol",
    "Ayo dong lanjut", "Siapa punya ide?", "Cerita donk temen", "Gas lah bro",
    "Sepi amat sih", "Ada yang online?", "Woi pada kemana?", "Gas rame dong",
    "Kabar semua gimana?", "Ada topik seru?", "Woi ngobrol yuk", "Sepi banget nih",
    "Halo ada yang?", "Woi masih pada?", "Ayo cerita dong", "Ada info baru?",
    "Gas bahas apa nih?", "Woi rame dong", "Ada yang nongol?", "Sepi kali ini",
    "Yuk ngobrol santai", "Ada gosip baru?", "Woi pada sibuk?", "Cerita seru dong",
]

# Opener khusus fallback kalau AI opening gagal - lebih beragam nadanya
FALLBACK_OPENERS = [
    "Woi sepi banget", "Ada yang online?", "Gas ngobrol yuk", "Sepi amat sih",
    "Woi pada kemana?", "Kabar semua gimana?", "Ada topik seru?", "Cerita dong siapa",
    "Yuk rame dikit", "Woi masih pada?", "Ada info baru?", "Sepi kali ini grup",
    "Gas bahas apa nih?", "Ada gosip baru?", "Woi ada yang?", "Halo pada kemana",
    "Sepi ih dari tadi", "Ayo dong seru dikit", "Woi jangan sepi", "Ada cerita seru?",
    "Kok pada diem sih", "Yuk gas ngobrol", "Ada yg mau cerita?", "Woi hidupin dong",
]

# ==================== HELPERS ====================

def pick_non_repeating(pool, history):
    """Ambil random item dari pool, hindari yang baru saja dipakai di history."""
    candidates = [x for x in pool if x not in history]
    if not candidates:
        candidates = pool  # kalau semua sudah pernah dipakai, reset
...
def print_stats():
    ...
    print(f"{C.GREEN}   🤖 via Gemini AI:{C.RESET} {stats['ai_openings']}")
    print(f"{C.GREEN}   📚 via Fallback DB:{C.RESET} {stats['fallback_openings']}")
...
# ==================== AI ENGINE - GEMINI FOKUS (BALAS CHAT) ====================
...
def generate_ai_response(sender_name, user_text, context_messages=None, retry=0):
    ...
    if retry > 3:
        ...
        fallback = pick_non_repeating(FALLBACK_RESPONSES, recent_replies)
    ...
    try:
        ...
        recently_used = ", ".join(list(recent_replies)[-10:]) if recent_replies else "belum ada"
...
JANGAN pakai kalimat yang barusan dipakai ini: {recently_used}
...
Balas singkat (2-3 kata) yang natural dan diverse:"""
        ...
        if response and response.text:
            ...
            if 1 <= word_count <= 6 and len(reply_text) < 60:
                ...
                # Anti-repeat check
                if reply_text in recent_replies:
                    logger.warning(f"⚠️  Reply repetitif ('{reply_text}'), retrying...")
                    time.sleep(0.5)
                    return generate_ai_response(sender_name, user_text, context_messages, retry + 1)
...
# ==================== AI ENGINE - GEMINI (SMART OPENING) ====================

OPENING_MOODS = [
    "nanya kabar grup dengan santai",
    "ngajak orang cerita/curhat",
    "komentar random soal keadaan grup yang sepi",
    "ngelempar topik obrolan ringan",
    "basa-basi kayak lagi mulai ngobrol sama temen",
    "nanya lagi pada sibuk apa",
    "ngajak gosip santai",
]

def generate_ai_opening(retry=0):
    """
    🤖 Generate opening line pakai Gemini AI - lebih pintar & variatif
    dibanding cuma comot random dari list statis.
    Tetap 2-3 kata, natural, dan tidak mengulang opening terakhir.
    """
    if retry > 2:
        logger.warning("AI opening gagal, pakai fallback opener DB")
        opener = pick_non_repeating(FALLBACK_OPENERS, recent_openers)
        return opener, True

    try:
        mood = random.choice(OPENING_MOODS)
        recently_used = ", ".join(list(recent_openers)[-8:]) if recent_openers else "belum ada"

        system_prompt = f"""Kamu adalah member grup Telegram Indonesia yang mau mulai ngobrol duluan
karena grup lagi sepi. WAJIB Bahasa Indonesia santai, gaya anak grup chat.
HANYA 2-3 KATA MAKSIMAL. Jangan panjang lebar, jangan pakai tanda baca berlebihan.
Nada yang diminta sekarang: {mood}.
JANGAN ulangi kalimat pembuka ini yang barusan dipakai: {recently_used}
Contoh gaya (bukan harus dipakai persis): "Woi sepi bat", "Ada yg online?", "Gas cerita dong", "Pada sibuk semua?", "Sepi amat sih", "Kabar semua gimana?"
Balas HANYA kalimat pembuka itu saja, tanpa penjelasan tambahan."""

        response = ai_client.models.generate_content(
            model='gemini-3.1-flash-lite',
            contents="Buatkan satu kalimat pembuka obrolan (2-3 kata) untuk grup yang lagi sepi:",
            config=types.GenerateContentConfig(
                system_instruction=system_prompt,
                temperature=1.0,
        ...
        if response and response.text:
            ...
            opener = opener.replace('**', '').replace('__', '').replace('```', '')
            opener = opener.replace('"', '').replace("'", '').strip()
            opener = ' '.join(opener.split())

            if opener.startswith('Balas:') or opener.startswith('Reply:'):
                opener = opener.split(':', 1)[1].strip()

            word_count = count_words(opener)

            if 1 <= word_count <= 4 and len(opener) < 50 and detect_language(opener):
                if opener in recent_openers:
                    logger.warning(f"⚠️  Opening repetitif ('{opener}'), retrying...")
                    time.sleep(0.5)
                    return generate_ai_opening(retry + 1)

                recent_openers.append(opener)
                logger.info(f"✅ [GEMINI OPENING] '{opener}' (mood: {mood})")
                return opener, False
            else:
                logger.warning(f"⚠️  Opening invalid ('{opener}'), retrying...")
                time.sleep(1)
                return generate_ai_opening(retry + 1)
        else:
            logger.warning("⚠️  Empty opening response from Gemini")
            time.sleep(1)
            return generate_ai_opening(retry + 1)

    except Exception as e:
        error_msg = str(e)
        logger.warning(f"⚠️  Gemini opening error: {error_msg[:100]}")
        if any(x in error_msg for x in ['500', '503', 'timeout', 'Timeout', 'DEADLINE', 'temporarily']):
            time.sleep(2 + retry)
            return generate_ai_opening(retry + 1)
        stats['errors'] += 1

    logger.warning("🚨 Gemini opening unavailable, pakai fallback opener DB")
    opener = pick_non_repeating(FALLBACK_OPENERS, recent_openers)
    return opener, True
...
# ==================== SMART OPENING (SEKARANG PAKAI AI) ====================

async def smart_open():
    """Kirim pesan pembuka - sekarang lebih pintar pakai Gemini AI dulu,
    fallback DB hanya kalau AI gagal, dan anti-pengulangan kalimat."""
    try:
        if should_exit:
            return

        opening, is_fallback = generate_ai_opening()

        if is_fallback:
            stats['fallback_openings'] += 1
            source_label = "FALLBACK DB"
        else:
            stats['ai_openings'] += 1
            source_label = "GEMINI AI"

        await client.send_message(TARGET_GROUP, opening, reply_to=TOPIC_ID)
        print(f"{C.GREEN}📢 [SELF-CHAT via {source_label}] {opening}{C.RESET}\n")
        logger.info(f"[SELF-CHAT/{source_label}] Sent: {opening}")
...
async def main():
    ...
    try:
        ...
        print(f"{C.CYAN}SELF-CHAT: AI-powered, anti-repeat, jika sepi 60+ detik{C.RESET}")
